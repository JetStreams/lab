...
public class MeshConverter {
    ...
    private static Triangle3D createTriangle(StlCoordinate coord1, StlCoordinate coord2, StlCoordinate coord3) {
        return new Triangle3D(toVec(coord1), toVec(coord2), toVec(coord3));
    }

    private static StlCoordinate[] getNextQuarter(List<StlCoordinate> stlCoordinates, int current) {
    ...
    private static Vec3D toVec(StlCoordinate coord) {
        return new Vec3D((float) coord.getX(), (float) coord.getY(), (float) coord.getZ());
    }

    public static List<Triangle3D> toTriangle(List<StlCoordinate> stlCoordinates) {
